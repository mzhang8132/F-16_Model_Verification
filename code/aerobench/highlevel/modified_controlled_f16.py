'''
Modifed Python Version of F-16 GCAS
...
def modified_controlled_f16(t, x_f16, u_ref, llc, f16_model='morelli', v2_integrators=False):
    'returns the LQR-controlled F-16 state derivatives and more'

    assert isinstance(x_f16, np.ndarray)
    assert isinstance(llc, modified_LowLevelController)
    assert u_ref.size == 5
